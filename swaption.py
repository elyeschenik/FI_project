...
class Cash_Settled_Swaption(Swaption):
    def __init__(self, pricing_date, start_date, end_date, expiry, curve_1, curve_2, isPayer, strike, fixed_freq, float_freq, fixed_convention, float_convention, vol, forward_convention, discount_convention, notional = 1000, spot_swap_rate = None, dates = None):
        ...
        if dates is None:
            self.dates = self.swap.fixed_dates
        else:
            self.dates = dates

    def Get_Level_Cash(self, s = None):
        N = int(self.forward_convention.coverage(self.start_date, self.end_date)) + 1
        f = self.fixed_freq
        if s is None:
            s = self.spot_swap_rate
    ...
    def PV(self):
        ...
        T = self.forward_convention.coverage(self.pricing_date, self.expiry)
        if self.isPayer:
            out = self.notional * self.get_DF(self.expiry, 1) * self.Level_Cash * self.BSClosedForm(self.spot_swap_rate, self.strike, 0, self.vol, T, True)
        else:
...
class Cash_Settled_Swaption_SABR(Cash_Settled_Swaption):
    def __init__(self, pricing_date, start_date, end_date, expiry, curve_1, curve_2, isPayer, strike, fixed_freq, float_freq, fixed_convention, float_convention, sigma_0, alpha, beta, rho, forward_convention, discount_convention, notional = 1000, spot_rate = None, spot_swap_rate_level = None, dates = None):
        super(Cash_Settled_Swaption_SABR, self).__init__(pricing_date, start_date, end_date, expiry, curve_1, curve_2, isPayer, strike, fixed_freq, float_freq, fixed_convention, float_convention, None, forward_convention, discount_convention, notional, spot_rate, dates)
